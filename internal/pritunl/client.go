...
type Client interface {
	...
	DeleteRouteFromServer(serverId string, routeId string) error
	UpdateRouteOnServer(serverId string, route Route) (*Route, error)
...
func (c client) AddRouteToServer(serverId string, route Route) (*Route, error) {
	...
	var respRoute Route
	err = json.Unmarshal(body, &respRoute)

	if err != nil {
		return nil, fmt.Errorf("GetRouteOnServer: %s: %+v, id=%s, body=%s", err, route, serverId, body)
	}

	return &route, nil
}

func (c client) UpdateRouteOnServer(serverId string, route Route) (*Route, error) {
	jsonData, err := json.Marshal(route)
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("UpdateRouteOnServer: Error on HTTP request: %s", err)
	}
	defer resp.Body.Close()

	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("Non-200 response on updating a route on the server\nbody=%s", body)
	}
...
func (c client) DeleteRouteFromServer(serverId string, routeId string) error {
	url := fmt.Sprintf("/server/%s/route/%s", serverId, routeId)
	req, err := http.NewRequest("DELETE", url, nil)
